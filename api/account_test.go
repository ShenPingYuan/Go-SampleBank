...
import (
	...
	mockdb "github.com/ShenPingYuan/go-webdemo/db/mock"
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				...
				assertBodyMatchAccount(assert, recorder.Body, account)
			},
		},
		{
			name:      "NotFound",
			...
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				assert.Equal(recorder.Code, http.StatusNotFound)
			},
		},
		{
			name:      "BadRequest",
...
func TestServer_getAccount(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.server.getAccount(tt.args.ctx)
		})
	}
}

func assertBodyMatchAccount(assert *assert.Assertions, body *bytes.Buffer, account db.Account) {
